...
public class RevueAfficherToutController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        try {
            Parent firstRow = FXMLLoader.load(getClass().getResource("/app/gui/back_end/societe/offre_de_travail/revue/TableRowModel.fxml"));
            tableau.getChildren().add(firstRow);
        } catch (IOException ex) {
            Logger.getLogger(RevueAfficherToutController.class.getName()).log(Level.SEVERE, null, ex);
        }

        List<Revue> listRevue = RevueCrud.getInstance().getAllRevues();
        listRevue.forEach((revue) -> {
            tableau.getChildren().add(creerLigneRevue(revue));
        });
    }

    private Parent creerLigneRevue(Revue revue) {
        try {
            Parent parent = FXMLLoader.load(getClass().getResource("/app/gui/back_end/societe/offre_de_travail/revue/TableRowModel.fxml"));

            CandidatureOffre candidatureOffre = CandidatureOffreCrud.getInstance().getCandidaturesOffreById(revue.getCandidatureOffreId());
            ...
            ((Text) parent.lookup("#textNbEtoiles")).setText(String.valueOf(revue.getNbEtoiles()));
            ((Text) parent.lookup("#textNbEtoiles")).setFont(Font.font(null, FontWeight.NORMAL, 12));
            ((Text) parent.lookup("#textNbEtoiles")).setWrappingWidth(70);
            ((Text) parent.lookup("#textDate")).setText(revue.getDateCreation().toLocalDateTime().toString());
    ...
    private void supprimerRevue(Revue revue) {
        Alert alert = new Alert(Alert.AlertType.CONFIRMATION);
        alert.setTitle("Confirmer la suppression");
        alert.setHeaderText(null);
        alert.setContentText("Etes vous sûr de vouloir supprimer votre revue ?");
        Optional<ButtonType> action = alert.showAndWait();

        if (action.get() == ButtonType.OK) {
            RevueCrud.getInstance().supprimerRevue(revue);
            MainWindowController.chargerInterface(
                    getClass().getResource("/app/gui/back_end/societe/offre_de_travail/revue/AfficherTout.fxml")
            );
        }
    }
}
